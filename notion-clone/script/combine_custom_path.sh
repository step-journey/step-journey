...
# 병합할 경로 목록
TARGET_PATHS=(
    "notion-clone/src/features/block"
)
...
for TARGET_PATH in "${TARGET_PATHS[@]}"; do
    ...
    if [[ "$TARGET_PATH" == /* ]]; then
        ...
        # notion-clone 경로로 시작하는지 확인하고 상대 경로로 변환
        ...
        # 상대 경로 또는 "notion-clone/"으로 시작하는 경로
        if [[ "$TARGET_PATH" == notion-clone/* ]]; then
            # "notion-clone/" 접두사 제거
            CLEAN_PATH="${TARGET_PATH#notion-clone/}"
    ...
    # 경로 정보 출력
    echo "## Files from: notion-clone/$RELATIVE_PATH" >> "$OUTPUT_FILE"

    # 경로가 파일인 경우 단일 파일만 포함
    if [ -f "$FULL_PATH" ]; then
        echo "### notion-clone/$RELATIVE_PATH:" >> "$OUTPUT_FILE"
    ...
    # 경로가 디렉토리인 경우 하위 파일 모두 포함
        ...
        do
            ...
            echo "### notion-clone/$file_relative:" >> "$OUTPUT_FILE"
