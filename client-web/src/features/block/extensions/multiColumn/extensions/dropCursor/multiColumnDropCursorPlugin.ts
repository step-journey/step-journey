    ...
    props: {
      handleDrop(view, event, slice, _moved) {
        ...
        if (blockInfo.blockNoteType === "column") {
          ...
          const filteredColumns = columnList.children
          ...
          const insertPosition = position === "left" ? index : index + 1;
          const newColumn = {
            type: "column",
          ...
          const newChildren = [
            ...filteredColumns.slice(0, insertPosition),
            newColumn,
            ...filteredColumns.slice(insertPosition),
          ];
