...
TARGET_PATHS=(
    "client-web/src/data/cubic-problem.json"
    "client-web/src/data/index.ts"
    "client-web/src/types/journey.ts"
    "client-web/src/pages/Journey/JourneyContent.tsx"
...
#echo "- public/ : 빌드 시 그대로 루트에 복사되어 정적으로 제공되는 파일(favicons, robots.txt 등)을 포함" >> "$OUTPUT_FILE"
#echo "- script/ : 프로젝트 빌드, 배포 및 유지보수 작업 자동화를 위한 쉘 스크립트를 관리" >> "$OUTPUT_FILE"
#echo "- src/assets/ : 이미지, JSON 데이터 등 정적 자산을 관리하며 컴파일 시 모듈로 임포트됨" >> "$OUTPUT_FILE"
#echo "- src/components/ui/ : 디자인 시스템(shadcn/ui) 컴포넌트를 포함" >> "$OUTPUT_FILE"
#echo "- src/components/common/ : 전역에서 사용하는 공통 컴포넌트를 포함" >> "$OUTPUT_FILE"
#echo "- src/constants/ : 전역적으로 공유되는 상수(API 설정, 경로, React Query 키 등)를 관리" >> "$OUTPUT_FILE"
#echo "- src/features/ : 도메인(기능) 기반으로 모듈화된 코드를 관리" >> "$OUTPUT_FILE"
#echo "- src/features/*/*.tsx : 각 도메인의 최상위 페이지 컴포넌트로 라우팅 진입점 역할" >> "$OUTPUT_FILE"
#echo "- src/features/**/components/ : 특정 도메인에서만 사용되는 컴포넌트를 포함" >> "$OUTPUT_FILE"
#echo "- src/features/**/hooks/ : 특정 도메인 전용 커스텀 훅을 포함" >> "$OUTPUT_FILE"
#echo "- src/features/**/services/ : 특정 도메인의 API 호출 및 관련 서비스 로직을 포함" >> "$OUTPUT_FILE"
#echo "- src/features/**/store/ : 특정 도메인의 상태 관리(Zustand 등)를 포함" >> "$OUTPUT_FILE"
#echo "- src/features/**/types/ : 특정 도메인의 타입 정의를 포함" >> "$OUTPUT_FILE"
#echo "- src/features/**/utils/ : 특정 도메인 전용 비즈니스 로직 관련 유틸 함수를 포함" >> "$OUTPUT_FILE"
#echo "- src/hooks/ : 도메인에 무관하게 공통적으로 사용되는 전역 커스텀 훅을 포함" >> "$OUTPUT_FILE"
#echo "- src/lib/ : 외부 라이브러리를 확장하거나 자체적으로 공통으로 사용하는 라이브러리화된 유틸 함수를 포함" >> "$OUTPUT_FILE"
#echo "- src/services/ : 전역에서 사용하는 서비스 로직(API 클라이언트, DB 연결 등)을 포함" >> "$OUTPUT_FILE"
#echo "- src/store/ : 앱 전체적으로 공유되는 전역 상태 관리를 포함" >> "$OUTPUT_FILE"
#echo "- src/types/ : 프로젝트 전역 공통 타입 정의를 포함" >> "$OUTPUT_FILE"
#echo "- src/utils/ : 프로젝트 전반에 걸쳐 사용하는 범용 유틸리티 함수 및 공통 로직을 포함" >> "$OUTPUT_FILE"
#echo "" >> "$OUTPUT_FILE"
